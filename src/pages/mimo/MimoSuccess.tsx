...
import { Check, Navigation, Phone, X } from "lucide-react";
...
export default function MimoSuccess() {
  ...
  const { cancelReservation } = useMimoData();
  ...
  const handleDirections = () => {
    if (!state?.salonLat || !state?.salonLng) return;
    window.open(
      `https://www.google.com/maps/dir/?api=1&destination=${state.salonLat},${state.salonLng}`,
      "_blank",
      "noopener,noreferrer",
    );
  };
  ...
  const handleCall = () => {
    if (!state?.salonPhone) return;
    window.location.href = `tel:${state.salonPhone}`;
  };

  const handleCancel = async () => {
    if (!state?.reservationId) return;
    setCancelling(true);
    await cancelReservation(state.reservationId);
    setCancelling(false);
    setCancelled(true);
  ...
  return (
    <div className="flex min-h-full flex-col items-center justify-center gap-8 bg-background px-6 pb-10">
      ...
      <div className="space-y-1 text-center">
        <h1 className="text-xl font-bold text-foreground">{cancelled ? "예약이 취소됐어요" : "예약 완료"}</h1>
        <p className="text-sm text-muted-foreground">
          {cancelled ? "다른 매장을 다시 둘러보세요." : "예약이 성공적으로 확정되었습니다."}
        </p>
      </div>
      ...
      {!cancelled && state && (
        <div className="grid w-full grid-cols-3 gap-2">
          <button
            ...
            disabled={!state.salonLat || !state.salonLng}
            className="flex flex-col items-center gap-1.5 rounded-2xl border border-border bg-card py-3 text-xs font-medium text-foreground disabled:opacity-40"
          >
          ...
          <button
            ...
            disabled={!state.reservationId || cancelling}
            className="flex flex-col items-center gap-1.5 rounded-2xl border border-border bg-card py-3 text-xs font-medium text-destructive disabled:opacity-40"
          >
            <X className="h-4.5 w-4.5" />
            {cancelling ? "취소 중..." : "예약취소"}
          </button>
        </div>
      )}

      <div className="w-full space-y-2">
        {!cancelled && <MimoPrimaryButton onClick={() => navigate("/mimo/bookings")}>예약 내역 보기</MimoPrimaryButton>}
